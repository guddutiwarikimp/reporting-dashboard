...
import { DEALS_DROPDOWN_FILTERS, DEALS_MOBILE_FILTERS, DEALS_FILTER_DRAWER_OPTIONS } from '@/constants/deals'
...
const DealsFilter = () => {
    ...
    const desktopMenuItems: MenuProps['items'] = useMemo(() => (
        DEALS_FILTER_DRAWER_OPTIONS.map((option, idx) => ({ key: idx + 1, label: option }))
    ), [])

    const mobileMenuItems: MenuProps['items'] = useMemo(() => (
        DEALS_MOBILE_FILTERS.map((item) => ({
            key: item.key,
            label: (
    ...
    const activeFilterTitle = useMemo(() => (
        DEALS_MOBILE_FILTERS.find(item => item.key === activeFilter)?.name
    ), [activeFilter])
    ...
    return (
        <>
            {DEALS_DROPDOWN_FILTERS.map((dropdown) => (
            ...
            >
                {activeFilter !== -1 && (
                    <div className="flex flex-col gap-2">
                        {DEALS_FILTER_DRAWER_OPTIONS.map((label, idx) => (
