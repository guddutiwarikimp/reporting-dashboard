...
interface LeadSourceMetricCardsProps {
  theme: Theme | undefined;
  timePeriod: string;
...
export default function LeadSourceMetricCards({ theme, timePeriod, onMetricSelect, selectedMetric, selectedQuery }: LeadSourceMetricCardsProps) {
  ...
  const metrics = LEAD_SOURCE_METRICS.map(metric => {
    ...
    else if (metricName.includes("Calls") || metricName.includes("Emails") || metricName.includes("Texts")) trend = "up";
    else if (metricName.includes("Notes") || metricName.includes("Tasks")) trend = "up";
